...
public class Screen extends PApplet implements Runnable{
    ...
    public Screen(Mouse mouse,Keyboard keyboard, Arena Arena, Data data){
    ...
    void game(){
        Tuple<Player,List<Player>> Players = Arena.getArena();
        if(Players.first == null)
            return;
        background(51);
        fill(255, 255, 255);
        circle(400,400,30);
        fill(Players.first.r,Players.first.g,Players.first.b,255);
        circle(400,400,25);
        for(Player p: Players.second){
            if (p.isPlayer) {
    ...
    public void run() {
        ...
        Screen screen = new Screen(this.mouse,this.keyboard, this.Arena, this.data);
