...
-export([start/1, start/0, parseGame/2]).
...
generatePowers(Powers, Players) ->
    Number = rand:uniform(100),
    if
        Number == 42 ->
            generatePowers(Players) ++ Powers;
        true ->
            Powers
    end.

generatePowers([]) -> [];
generatePowers([{_Player, {_From, _Color, {Px, Py}, _Angle, _Speed,_N_Win}} | T]) ->
    ...
    [{X + Px, Y + Py, Color} | generatePowers(T)].

gameTimer(Players, Powers) ->
    Self = self(),
    spawn(fun() -> receive after 40 -> Self ! timeout end end), % tickrate
    {NewPlayers, TempPowers} = handleGame(Players, Powers),
    NewPowers = generatePowers(TempPowers, maps:to_list(Players)),
    PlayerInfo = parseGame(maps:to_list(NewPlayers), []),
    case NewPowers of
        [] -> Info = string:concat(PlayerInfo, "\n");
        _ ->
            PowerInfo = parsePowers(NewPowers, []),
            Info = string:join([PlayerInfo, PowerInfo], "|")
    end,
    [From ! Info || {_Player, {From, _Color, _Pos, _Angle, _Speed,_N_Win}} <- maps:to_list(NewPlayers)],
    gameLoop(NewPlayers, NewPowers).

gameLoop(Players, Powers) ->
    receive
        timeout ->
            gameTimer(Players, Powers);
        {leave, Username, From} ->
            From ! leave_done,
            handleRemovePlayer(Players, Username, Powers);
        {Info, _From} ->
            NewPlayers = handleInput(Players, Info),
            gameLoop(NewPlayers, Powers)
    end.
...
handleInput(Players, {Username, Key}) -> 
    ...
    case Res of
        ...
        {From, Color, {OldX, OldY}, Angle, Speed,N_Win} ->
            ...
            {DirX, DirY} = normalize(math:cos(NewAngle), math:sin(NewAngle)),
            {X, Y} = {OldX + DirX * NewSpeed, OldY + DirY * NewSpeed},
            maps:update(Username, {From, Color, {X, Y}, NewAngle, NewSpeed,N_Win}, Players)
    end.


handleGame(Players, Powers) ->
    PowerCollisions = findPowerCollisions(Powers, maps:to_list(Players)),
    PlayerCollisions  = findColisions(maps:to_list(Players)),
    {NewPlayers, FinalPowers} = handlePowerCollisions(PowerCollisions, Players, Powers),
    FinalPlayers = handlePlayerCollisions(PlayerCollisions, NewPlayers),
    {FinalPlayers, FinalPowers}.

handlePowerCollisions([], Players, Powers) -> {Players, Powers};
handlePowerCollisions([{Player, X, Y, Color} | T], Players, Powers) ->
    {NewPlayers, NewPowers} = handlePowerCollisions(T, Players, Powers),
    ...
    {maps:update(Player, {From, Color, Pos, Angle, NewSpeed,N_Win_Match}, NewPlayers), [{CX, CY, CColor} || {CX, CY, CColor} <- NewPowers, not ((CX == X) and (CY == Y))]}.
...
findPowerCollisions([], _Players) -> [];
findPowerCollisions([H | T], Players) -> findPowerCollisionsAux(H, Players) ++ findPowerCollisions(T, Players).

findPowerCollisionsAux(_C, []) -> [];
findPowerCollisionsAux(C, [H | T]) ->
    {CX, CY, CColor} = C,
    {Player, {_From, _Color, HPos, _HAngle, _Speed, _N_Win}} = H,
    case geometry:powerInsideCircle({{CX, CY}, 10}, {HPos, 30}) of
        true -> [{Player, CX, CY, CColor} | findPowerCollisionsAux(C, T)];
        _ -> findPowerCollisionsAux(C, T)
    end.
...
parsePowers([], List) -> string:concat(string:join(List, "|"), "\n");
parsePowers([{X, Y, Color} | Tail], List) ->
    InfoPower = string:join(["<>", atom_to_list(Color), float_to_list(X), float_to_list(Y), "1.23", "0"], " "),
    parsePowers(Tail, [InfoPower | List]).
